...
<template>
    <BaseModal title="家計簿追加" :isOpen="isOpenModal" @closeModal="$emit('closeModal')"
    verticalPosition="top-20"
    >
        <template #modalBody>
            <TheForm label="家計簿名">
                <input type="text" class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />  
            </TheForm>
            <TheForm label="説明">
        ...
        <template #buttons>
            <PrimaryButton @click="$emit('addHousehold')">
                追加
            </PrimaryButton>
