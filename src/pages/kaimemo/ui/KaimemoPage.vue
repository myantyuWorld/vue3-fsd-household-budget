  ...
  <div>
    ...
    <div class="">
      <div v-for="item in filteredItems" :key="item.id"
        class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 m-2">
        <ShoppingMemoItem :shoppingMemo="item" @click="onClickArchiveItem" />
      </div>
    </div>
      ...
      <template #modalBody>
        <TheForm label="品名">
          <input type="text"
            ...
            :class="{ 'bg-red-50 border-red-500': errors.name }" v-model="name" v-bind="nameProps" />
          <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ errors.name }}</p>
        </TheForm>
        <TheForm label="カテゴリ">
          <select
            class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 mt-1"
            :class="{ 'bg-red-50 border-red-500': errors.tag }" v-model="tag" v-bind="tagProps">
            <template v-for="categoryLimit in selectedCategoryLimit" :key="categoryLimit.category.id">
