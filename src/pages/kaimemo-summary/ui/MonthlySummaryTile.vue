    ...
    <div class="w-[calc(100%-16px)] h-max rounded text-center p-3">
      <div class="flex items-center justify-center gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="text-2xl font-bold text-red-500 dark:text-red-400">{{ formattedAvailableAmount }}</span>
          <span class="text-xl text-gray-600 dark:text-gray-400">円</span>
        </div>
      </div>
      <div class="relative">
        <span class="text-7xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-pink-600 to-purple-600 dark:from-indigo-400 dark:via-pink-400 dark:to-purple-400">
          {{ formattedCurrentExpense }}
          <span class="text-4xl">円</span>
        </span>
        <div class="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-indigo-500 via-pink-500 to-purple-500 rounded-full"></div>
      </div>
    </div>
  </TheCard>
</template>
