...
const { defineField, errors, onClickCreateInformation } = useCreateInformation()
...
<template>
  <BaseModal :isOpen="isOpen" title="お知らせ追加" @close="emit('close')" class="backdrop-blur-md">
    <template #modalBody>
      <div class="bg-gradient-to-br from-primary-bg to-white/50 rounded-2xl">
        <TheForm label="タイトル">
          <input
            ...
            :class="{ 'border-red-500 bg-red-50/80': errors.title }"
            v-model="title"
            v-bind="titleProps"
          />
        ...
        <TheForm label="内容">
          <input
            class="w-full p-4 border border-primary-light rounded-xl focus:border-primary focus:ring-2 focus:ring-primary-light bg-white/90 text-base"
            :class="{ 'border-red-500 bg-red-50/80': errors.content }"
            v-model="content"
            v-bind="contentProps"
          />
        ...
        <TheForm label="カテゴリ">
          <select
            class="w-full p-4 border border-primary-light rounded-xl focus:border-primary focus:ring-2 focus:ring-primary-light bg-white/90 text-base"
            :class="{ 'border-red-500 bg-red-50/80': errors.category }"
            v-model="category"
